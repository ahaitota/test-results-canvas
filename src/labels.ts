...
export function labelForPath(abs: string, discovered: Map<string, string>, localNames: string[] = []): string {
    for (const [label, p] of discovered) if (p === abs) return label;
    const name = basename(abs);
    const taken = (l: string) => discovered.has(l) || localNames.includes(l);
