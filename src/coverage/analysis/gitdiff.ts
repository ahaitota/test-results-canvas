...
function createGitExec(root: string): GitExec {
    return (args: string[]) => {
        try {
...
export interface FileChanges {
    // Repo-relative and forward-slashed, as git reports it.
    path: string;
    absPath: string;
    // Line numbers added or modified in the new version of the file.
    lines: Set<number>;
    // A new or untracked file, where every executable line counts as new.
    all: boolean;
    // Filled in only for untracked files: they have no diff to count.
    lineCount?: number;
}

export interface DiffResult {
    root: string;
    // Shown in the UI so the percentage is never ambiguous.
    against: string;
    files: FileChanges[];
}
