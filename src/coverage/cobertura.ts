...
function parseConditionCoverage(raw: string | undefined): BranchTotals | null {
    const m = /\((\d+)\s*\/\s*(\d+)\)/.exec(String(raw || ""));
    if (!m) return null;
    const covered = Number(m[1]);
    const total = Number(m[2]);
    if (!Number.isFinite(covered) || !Number.isFinite(total) || total <= 0) return null;
    return { covered, total };
}

export function parseCobertura(xml: string): CoverageReport {
    ...
    for (const tag of scanTags(text)) {
        ...
        if (name === "source" && !tag.closing && !tag.selfClosing) {
            ...
            if (value) sourceRoots.push(value);
            continue;
        }
