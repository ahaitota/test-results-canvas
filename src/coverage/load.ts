...
import type { DiffOptions, DiffResult, FileChanges } from "./gitdiff.js";
...
import type { CoverageFileSummary, CoveragePayload } from "./payload.js";
...
export interface LoadedCoverage {
    ...
    projectRoot?: string;
    // Kept server-side (Sets don't serialize) to mark changed lines in the
    // source view.
...
export interface LoadOptions {
    // Used to resolve report-relative paths and to run git. Derived from the
    // report's own location when omitted.
    projectRoot?: string;
    // Override for tests.
    diff?: DiffOptions;
    // Skip git entirely (the e2e suite runs against fixtures, where the
    // surrounding repository's diff would be noise).
    skipGit?: boolean;
    // Keep every line the report named coverable, including comments. Only for
