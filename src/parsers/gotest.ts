...
export function parseGoTest(text: string): TestResult[] {
    ...
    for (const event of jsonLines(text)) {
        ...
        if (!test) {
            ...
            if (action === "pass" || action === "fail" || action === "skip" || action === "build-fail") packagesDone.add(pkg);
    ...
    const failedInPackage = new Set<string>();
    for (const row of rows.values()) {
        if (row.status === "fail" && row.suite) failedInPackage.add(row.suite);
    }
    ...
    for (const pkg of packages) {
        if (!packagesDone.has(pkg)) throw new SyntaxError(`go test stream ends before ${pkg} finished`);
    }
    ...
    for (const [pkg, at] of packageFailed) {
        if (failedInPackage.has(pkg)) continue;
        rows.set(`${pkg}\u0000`, {
            name: pkg,
            status: "fail",
