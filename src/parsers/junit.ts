...
function attr(tag: string | undefined, name: string): string | undefined {
    ...
    for (let m = ATTR_PAIR.exec(text); m; m = ATTR_PAIR.exec(text)) {
        if (m[1] === name) return xmlUnescape(m[2] ?? m[3]);
    }
    return undefined;
}

// JUnit "time" is seconds (float) -> milliseconds.
function timeToMs(t: string | undefined): number | undefined {
...
function matchOpenTag(text: string, i: number, name: string): { attrs: string; end: number; selfClosing: boolean } | null {
...
function matchCloseTag(text: string, i: number, name: string): number {
...
function findCloseTag(text: string, from: number, name: string): { start: number; after: number } {
    let i = from;
    while (i < text.length) {
        const lt = text.indexOf("<", i);
        if (lt < 0) break;
...
function findChild(inner: string, names: readonly string[]): { attrs: string; body: string } | null {
    let i = 0;
    while (i < inner.length) {
        ...
        for (const name of names) {
            ...
            if (!tag) continue;
            if (tag.selfClosing) return { attrs: tag.attrs, body: "" };
