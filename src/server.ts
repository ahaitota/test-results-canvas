...
export async function createResultsServer(options: ResultsServerOptions = {}) {
    ...
    let groupDef: { name: string; members: { path: string; format?: string; mtimeMs: number }[] } | null = null;
    ...
    function applySources(list: SourceEntry[], name: string | null): void {
        ...
        if (name) groupDef = { name, members: list.map((e) => ({ path: e.source.path, format: e.format, mtimeMs: e.mtimeMs ?? 0 })) };
        rebuild();
        ...
        if (here.length === 1) {
            ...
            const follow = (candidates: readonly string[]): boolean => {
                for (const candidate of canonicalResultPaths(candidates)) {
                    if (heldByOthers.size && candidate !== before && heldByOthers.has(canonicalPath(candidate))) continue;
    ...
    function restoreGroup(def: { name: string; members: { path: string; format?: string; mtimeMs: number }[] }): boolean {
        ...
        const paths = def.members.map((m) => {
            if (existsSync(m.path)) return m.path;
        ...
        if (built.skipped.length || !built.entries.length) return false;
