...
import { serializeTrx, parseTrx } from "./parsers/trx.mjs";
import { parseJUnit } from "./parsers/junit.mjs";
...
export function newestResultsFileIn(dir) {
    let best = null, bestMtime = -1, names = [];
...
// Load view.mjs fresh each call (cache-busted) so UI edits show on refresh.
...
export async function createResultsServer(options = {}) {
    ...
    function watchDir(dir) {
        ...
        try {
            ...
            console.error(`[server] watch failed for ${dir}:`, err?.message || err);
        }
    }
    ...
    const server = createServer((req, res) => {
        ...
        renderShell(title)
            ...
            .catch((err) => { res.writeHead(500, { "Content-Type": "text/plain; charset=utf-8" }); res.end(`View error:\n${err?.stack || err}`); });
    });

    // Prefer the requested port for a stable URL; fall back to an ephemeral one.
    const boundPort = await new Promise((resolve) => {
        const onError = () => { server.removeListener("error", onError); server.listen(0, "127.0.0.1", () => resolve(server.address().port)); };
        server.once("error", onError);
        server.listen(port || 0, "127.0.0.1", () => { server.removeListener("error", onError); resolve(server.address().port); });
    });
