...
// Key a payload against the previous one, carrying `seq` across calls.
function reconcile(next: TestResult[], prev: TestResult[], prevKeys: string[], seq: Map<string, number>) {
  return reconcileRowKeys(next, prev, prevKeys, seq);
}
...
test("a fresh payload gives every row a unique key, even fully identical rows", () => {
  ...
  const { keys } = reconcile(rows, [], [], new Map());
...
test("an unchanged payload keeps every key", () => {
  ...
  const k1 = reconcile(p1, [], [], seq).keys;
  const p2 = [row({ name: "A", className: "C" }), row({ name: "B", className: "C" })];
  const k2 = reconcile(p2, p1, k1, seq).keys;
...
test("an ambiguous volatile change collapses expansion instead of transferring it", () => {
  ...
  const { keys: k2, reused } = reconcile(p2, p1, k1, seq);
...
test("switching to an unrelated payload drops every expanded key", () => {
  ...
  const { reused } = reconcile(p2, p1, k1, seq);
