...
parser claims — or one that claims a format but is incomplete: XML a conforming
parser would reject, a JSONL event line that will not parse, an event stream that
ends with a test still running, or an Allure folder with an unreadable result —
is ignored rather than shown as an empty or partly green passing run.
