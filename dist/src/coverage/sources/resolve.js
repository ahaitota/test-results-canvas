...
function createContainment(projectRoot) {
    ...
    return (candidate) => {
        ...
        if (!withinRoot(trusted, real, process.platform === "win32"))
            return undefined;
        return candidate;
    };
}
...
function createSourceResolver(options = {}) {
    ...
    const mayTouch = (candidate) => !lexicalRoot || withinRoot(lexicalRoot, resolvePath(candidate), process.platform === "win32");
    ...
    const reachable = (candidate) => mayTouch(candidate) && existsFile(candidate);
