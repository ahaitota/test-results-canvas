// Stylesheet for the Test Results UI, shared by both hosts.
//
// BASE_CSS is host-neutral: it only ever reads the --fgColor-*/--bgColor-*/
// --borderColor-*/--fontStack-* aliases. Each host prepends its own theme block
// that maps those aliases onto the tokens it actually exposes, so a single
// stylesheet renders correctly in the Copilot app and in a VS Code webview.
// Copilot app: documented app tokens (create-canvas contract) drive the main
// surface, so the exact palette (incl. Dark Dimmed, high-contrast, colorblind)
// is matched; our hex is the fallback and also sets the undocumented colors
// (success green, attention amber, muted backgrounds) per the resolved
// light/dark tone. data-theme is set from the detected app tone (see theme.ts).
export const THEME_COPILOT = `
  :root {
...
`;
// VS Code: the editor injects every theme colour as a --vscode-* variable and
// flips them itself, so there is no light/dark duplicate and no tone probing.
// Tints have no --vscode-* equivalent, so they are mixed from the status colour.
export const THEME_VSCODE = `
  :root {
    --fgColor-default: var(--vscode-foreground);
    --fgColor-muted: var(--vscode-descriptionForeground);
    --fgColor-accent: var(--vscode-textLink-foreground);
    --fgColor-success: var(--vscode-testing-iconPassed);
    --fgColor-attention: var(--vscode-testing-iconSkipped);
    --fgColor-danger: var(--vscode-testing-iconFailed);
    --bgColor-default: var(--vscode-sideBar-background, var(--vscode-editor-background));
    --bgColor-muted: var(--vscode-editorWidget-background);
    --button-default-bgColor-hover: var(--vscode-toolbar-hoverBackground);
    --bgColor-inset: var(--vscode-textCodeBlock-background);
    --bgColor-accent-muted: color-mix(in srgb, var(--fgColor-accent) 12%, transparent);
    --bgColor-success-muted: color-mix(in srgb, var(--fgColor-success) 12%, transparent);
    --bgColor-attention-muted: color-mix(in srgb, var(--fgColor-attention) 12%, transparent);
    --bgColor-danger-muted: color-mix(in srgb, var(--fgColor-danger) 12%, transparent);
    --borderColor-default: var(--vscode-widget-border, color-mix(in srgb, var(--fgColor-muted) 35%, transparent));
    --borderColor-muted: color-mix(in srgb, var(--fgColor-muted) 25%, transparent);
    --fontStack-sans: var(--vscode-font-family);
    --fontStack-mono: var(--vscode-editor-font-family);
  }
  /* A side-bar view is far narrower than a canvas panel. */
  body { padding:10px; max-width:none; font-size:var(--vscode-font-size); }
  h1 { font-size:15px; }
`;
