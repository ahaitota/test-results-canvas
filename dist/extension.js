...
const TEST_CMD_RE = /\b(dotnet\s+test|vstest|npm\s+(run\s+)?test|yarn\s+test|pnpm\s+test|jest|vitest|mvn\s+(test|verify)|gradle\w*\s+test|pytest|py\.test|go\s+test|cargo\s+test|nextest|rspec|phpunit|ctest|prove|dart\s+test|flutter\s+test)\b|--junitxml|--?logger[= ]?["']?trx|surefire|failsafe|\.trx\b/i;
        ...
        for (const ent of entries) {
            ...
            try {
                ...
                if (st.mtimeMs <= sinceMs)
                    continue;
                if (!looksLikeResults(readHead(abs)))
                    continue;
...
function surfaceIfResults(input) {
    try {
        ...
        const open = merged
            ...
            : { resultsFile: abs };
        if (!merged && coverageFile)
            open.coverageFile = coverageFile;
...
joined.session = await joinSession({
    canvases: [
        createCanvas({
            ...
            description: "A visual test-results and code-coverage canvas that renders .NET TRX, JUnit/NUnit/" +
            ...
            inputSchema: {
                ...
                properties: {
                    resultsFile: {
                        type: "string",
                        description: "Absolute path to a test-results file (.trx, JUnit/NUnit/xUnit/TestNG/CTest " +
                    ...
                    resultsFiles: {
                        type: "array",
                        items: { type: "string" },
                        description: "Absolute paths to several test-results files, merged into one run. Use this " +
                    ...
                    name: {
                        type: "string",
                        description: "Display name for a merged run, e.g. \"AITestAgentTests\". Only meaningful " +
            ...
            actions: [
                ...
                {
                    name: "open_files",
                    description: "Merge several test-results files into one run and show them as a single named " +
                    ...
                    handler: async (ctx) => {
                        ...
                        if (!merged.ok)
                            return { success: false, error: merged.error, skipped: merged.skipped };
                        return {
                            success: true,
                ...
                },
                {
                    ...
                    description: "Mark the tests a code change may affect. Use this to answer the canvas's " +
                    ...
                    handler: async (ctx) => {
                        ...
                        const refs = raw.map(asAgentTestRef).filter((r) => r !== null);
